...
import org.sqldm.service.MetricApprovalService;
import org.sqldm.service.MetricEngagementService;
import org.sqldm.service.MetricService;
import org.sqldm.service.MetricVersionAuditService;
...
public class MetricController {
    ...
    public ResponseEntity<Map<String, Object>> deleteMetric(@PathVariable("id") Long id) {
        Map<String, Object> response = new HashMap<>();
        try {
            boolean pending = metricService.deleteMetric(id);
            response.put("success", true);
            response.put("pendingApproval", pending);
            response.put("message", pending ? "删除申请已提交，等待审批" : "指标删除成功");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", e.getMessage());
    ...
    public ResponseEntity<Map<String, Object>> getAllMetrics(
        ...
        Map<String, Object> response = new HashMap<>();
        List<MetricDefinition> metrics = metricService.listMetrics(keyword, topicId, status, sort);
        response.put("success", true);
    ...
    public ResponseEntity<Map<String, Object>> searchMetrics(
            ...
            @RequestParam(value = "sort", required = false) String sort) {
        Map<String, Object> response = new HashMap<>();
        List<MetricDefinition> metrics = metricService.queryMetrics(keyword, topicId, status, sort);
        response.put("success", true);
        response.put("data", metrics);
        response.put("total", metrics.size());
        return ResponseEntity.ok(response);
