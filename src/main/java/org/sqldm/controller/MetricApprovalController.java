...
public class MetricApprovalController {
    ...
    @PostMapping("/delete/{requestId}/approve")
    public ResponseEntity<Map<String, Object>> approveDelete(
            @PathVariable("requestId") Long requestId,
            @RequestBody(required = false) Map<String, String> body) {
        Map<String, Object> response = new HashMap<>();
        try {
            String comment = body != null ? body.get("comment") : null;
            metricService.approveDelete(requestId, comment);
            response.put("success", true);
            response.put("message", "已通过删除申请");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", e.getMessage());
    ...
    @PostMapping("/delete/{requestId}/reject")
    public ResponseEntity<Map<String, Object>> rejectDelete(
            @PathVariable("requestId") Long requestId,
            @RequestBody(required = false) Map<String, String> body) {
        Map<String, Object> response = new HashMap<>();
        try {
            String comment = body != null ? body.get("comment") : null;
            metricService.rejectDelete(requestId, comment);
            response.put("success", true);
            response.put("message", "已驳回删除申请");
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", e.getMessage());
