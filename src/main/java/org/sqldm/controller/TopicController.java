...
import org.sqldm.service.TopicService;
...
public class TopicController {
    ...
    public ResponseEntity<?> getManageableTopics() {
        try {
            List<Topic> topics = topicService.getManageableTopics();
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "data", topics
            ));
    ...
    public ResponseEntity<?> getMyTopics() {
        try {
            List<Topic> topics = topicService.getMyTopics();
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "data", topics
            ));
    ...
    @GetMapping("/{topicId}/users")
    public ResponseEntity<?> getTopicUsers(@PathVariable Long topicId) {
        try {
            List<Map<String, Object>> rows = topicService.getTopicUsersDetail(topicId);
            return ResponseEntity.ok(Map.of(
                    "success", true,
                    "data", rows
            ));
